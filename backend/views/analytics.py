...
def revenue_analytics():
    ...
    total_revenue = db.session.query(func.sum(Order.total_price)).scalar() or 0  # Handle potential None

    # Revenue by product category (optional breakdown)
    revenue_by_category = (
        db.session.query(Product.category, func.sum(Order.total_price).label('category_revenue'))
        .join(OrderItem, Product.id == OrderItem.product_id)
    ...
    category_breakdown = [
      {"category": cat, "revenue": rev or 0} for cat, rev in revenue_by_category
    ]
